...
struct bu21029_ts_data {
	...
	struct gpio_desc		*level_shift_gpios;
	u32				x_plate_ohms;
	u32 min_x;
	u32 max_x;
	u32 min_y;
...
static void bu21029_touch_report(struct bu21029_ts_data *bu21029, const u8 *buf)
{
	...
	if (z1 && z2) {
		...
		if (rz <= max_pressure) {
			if (bu21029->invert_x)
				x = bu21029->min_x + bu21029->max_x - x;
			if (bu21029->invert_y)
				y = bu21029->min_y + bu21029->max_y - y;
...
static void bu21029_touch_release(unsigned long data)
{
	struct bu21029_ts_data *bu21029 = (struct bu21029_ts_data *)data;
			 ...
			 const struct i2c_device_id *id)
{
	...
	/*
	 * 3.18 backport deviation: the DT-X400's panel sits behind a level
	 * shifter enabled via a PCA9535 expander pin. 3.18 lacks gpio-hog
	 * support, so the driver owns the pin and holds it high.
	 */
	bu21029->level_shift_gpios = devm_gpiod_get_optional(&client->dev,
						"level-shift", GPIOD_OUT_HIGH);
	if (IS_ERR(bu21029->level_shift_gpios)) {
		error = PTR_ERR(bu21029->level_shift_gpios);
		if (error != -EPROBE_DEFER)
			dev_err(&client->dev,
				"failed to acquire 'level-shift' gpio: %d\n",
				error);
		return error;
	}
	...
	__set_bit(INPUT_PROP_DIRECT, in_dev->propbit);
	
	/*
	 * 3.18 backport deviation: touchscreen_parse_properties() does not
	 * exist yet; parse the standard touchscreen DT properties by hand.
	 * Calibration values (measured panel window, axis inversion) live
	 * in the devicetree.
	 */
	{
		u32 min_x = 0, min_y = 0, size_x = 0, size_y = 0;
		u32 max_x = MAX_12BIT, max_y = MAX_12BIT;
		struct device_node *np = client->dev.of_node;

		of_property_read_u32(np, "touchscreen-min-x", &min_x);
		of_property_read_u32(np, "touchscreen-min-y", &min_y);
		if (!of_property_read_u32(np, "touchscreen-size-x", &size_x))
			max_x = min_x + size_x - 1;
		if (!of_property_read_u32(np, "touchscreen-size-y", &size_y))
			max_y = min_y + size_y - 1;
		...
		input_set_abs_params(in_dev, ABS_X, min_x, max_x, 0, 0);
		input_set_abs_params(in_dev, ABS_Y, min_y, max_y, 0, 0);
	}
